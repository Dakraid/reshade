    ...
    , _app_state(device)
{
    ...
    subscribe_to_load_config([this](const ini_file& config) {
        ...
        config.get("DX11_BUFFER_DETECTION", "UseAlternativeClearSelector", _buffer_detection->alternative_clear_selector);
        config.get("DX11_BUFFER_DETECTION", "DepthBufferAltIndex", _buffer_detection->depth_buffer_alt_index);
    ...
    subscribe_to_save_config([this](ini_file& config) {
        ...
        config.set("DX11_BUFFER_DETECTION", "UseAlternativeClearSelector", _buffer_detection->alternative_clear_selector);
        config.set("DX11_BUFFER_DETECTION", "DepthBufferAltIndex", _buffer_detection->depth_buffer_alt_index);
    });
#endif
}
...
bool reshade::d3d11::runtime_d3d11::on_init(const DXGI_SWAP_CHAIN_DESC& swap_desc)
{
    ...
    if (swap_desc.SampleDesc.Count > 1 || make_dxgi_format_normal(_backbuffer_format) != _backbuffer_format || !is_windows7) {
...
bool reshade::d3d11::runtime_d3d11::init_effect(size_t index)
{
    ...
    if (_d3d_compiler == nullptr) {
        LOG(ERROR) << "Unable to load HLSL compiler (\"d3dcompiler_47.dll\")."
                   << " Make sure you have the DirectX end-user runtime (June 2010) installed or a newer version of the library in the application directory.";
        return false;
    }
    ...
    const UINT max_uav_bindings = _renderer_id >= D3D_FEATURE_LEVEL_11_1 ? D3D11_1_UAV_SLOT_COUNT : _renderer_id == D3D_FEATURE_LEVEL_11_0 ? D3D11_PS_CS_UAV_REGISTER_COUNT : _renderer_id >= D3D_FEATURE_LEVEL_10_0 ? D3D11_CS_4_X_UAV_REGISTER_COUNT : 0;
    ...
    for (const reshadefx::sampler_info& info : effect.module.samplers) {
        if (info.binding >= D3D11_COMMONSHADER_SAMPLER_SLOT_COUNT) {
            LOG(ERROR) << "Cannot bind sampler '" << info.unique_name << "' since it exceeds the maximum number of allowed sampler slots in "
                       << "D3D11"
                       << " (" << info.binding << ", allowed are up to " << D3D11_COMMONSHADER_SAMPLER_SLOT_COUNT << ").";
            return false;
        }
        if (info.texture_binding >= D3D11_COMMONSHADER_INPUT_RESOURCE_SLOT_COUNT) {
            LOG(ERROR) << "Cannot bind texture '" << info.texture_name << "' since it exceeds the maximum number of allowed resource slots in "
                       << "D3D11"
                       << " (" << info.texture_binding << ", allowed are up to " << D3D11_COMMONSHADER_INPUT_RESOURCE_SLOT_COUNT << ").";
            return false;
        }

        const texture& texture = look_up_texture_by_name(info.texture_name);

        technique_init.srv_bindings[info.texture_binding] = static_cast<d3d11_tex_data*>(texture.impl)->srv[info.srgb ? 1 : 0];
    ...
    for (const reshadefx::storage_info& info : effect.module.storages) {
        if (info.binding >= max_uav_bindings) {
            LOG(ERROR) << "Cannot bind storage '" << info.unique_name << "' since it exceeds the maximum number of allowed resource slots in "
                       << "D3D11"
                       << " (" << info.binding << ", allowed are up to " << max_uav_bindings << ").";
            return false;
        }
            ...
            if (!pass_info.cs_entry_point.empty()) {
...

                {
                    ...
                    const auto convert_blend_func = [](reshadefx::pass_blend_func value) {
                        ...
                        case reshadefx::pass_blend_func::inv_src_color:
                            return D3D11_BLEND_INV_SRC_COLOR;
                        case reshadefx::pass_blend_func::inv_src_alpha:
                            return D3D11_BLEND_INV_SRC_ALPHA;
                        case reshadefx::pass_blend_func::dst_color:
                            return D3D11_BLEND_DEST_COLOR;
                        case reshadefx::pass_blend_func::dst_alpha:
                            return D3D11_BLEND_DEST_ALPHA;
                        case reshadefx::pass_blend_func::inv_dst_color:
                            return D3D11_BLEND_INV_DEST_COLOR;
                        case reshadefx::pass_blend_func::inv_dst_alpha:
...

                {
                    ...
                    const auto convert_stencil_op = [](reshadefx::pass_stencil_op value) {
                        ...
                        case reshadefx::pass_stencil_op::invert:
...
void reshade::d3d11::runtime_d3d11::render_technique(technique& technique)
{
    ...
    if (impl->query_in_flight) {
        ...
        if (_immediate_context->GetData(impl->timestamp_disjoint.get(), &disjoint, sizeof(disjoint), D3D11_ASYNC_GETDATA_DONOTFLUSH) == S_OK && _immediate_context->GetData(impl->timestamp_query_beg.get(), &timestamp0, sizeof(timestamp0), D3D11_ASYNC_GETDATA_DONOTFLUSH) == S_OK && _immediate_context->GetData(impl->timestamp_query_end.get(), &timestamp1, sizeof(timestamp1), D3D11_ASYNC_GETDATA_DONOTFLUSH) == S_OK) {
    ...
    for (size_t pass_index = 0; pass_index < technique.passes.size(); ++pass_index) {
        ...
        for (const com_ptr<ID3D11ShaderResourceView>& resource : pass_data.modified_resources) {
            if (resource == _backbuffer_texture_srv[0] || resource == _backbuffer_texture_srv[1]) {
...
void reshade::d3d11::runtime_d3d11::update_alt_depth_texture_bindings(com_ptr<ID3D11Texture2D> depth_texture)
{
    ...
    // Update all references to the new texture
    for (const texture& tex : _textures) {
        if (tex.impl == nullptr || tex.impl_reference != texture_reference::depth_buffer_alt)
            continue;
        const auto tex_impl = static_cast<d3d11_tex_data*>(tex.impl);
