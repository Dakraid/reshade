    ...
    for (std::filesystem::path search_path : search_paths)
        if (resolve_path(search_path))
            for (const std::filesystem::directory_entry& entry : std::filesystem::directory_iterator(search_path, std::filesystem::directory_options::skip_permission_denied, ec))
                if (!entry.is_directory(ec) && std::find(extensions.begin(), extensions.end(), entry.path().extension()) != extensions.end())
...
bool reshade::runtime::load_effect(const std::filesystem::path& path, size_t index)
{
    ...
    for (texture texture : effect.module.textures) {
        ...
        else if (texture.semantic == "DEPTH_ALT")
            texture.impl_reference = texture_reference::depth_buffer_alt;
...
void reshade::runtime::load_effects()
{
    ...
    const std::vector<std::filesystem::path> effect_files = find_files(_effect_search_paths, { L".fx" });
    ...
    if (_reload_remaining_effects == 0) {
        ...
        if (_show_code_editor && !_editor_file.empty()) {
            if (const auto it = std::find_if(_effects.begin(), _effects.end(),
                    [this](const effect& fx) { return fx.source_file == _editor_file; });
                it != _effects.end())
...
void reshade::runtime::load_current_preset()
{
    ...
    std::sort(_techniques.begin(), _techniques.end(),
        [&sorted_technique_list](const technique& lhs, const technique& rhs) {
            return (std::find(sorted_technique_list.begin(), sorted_technique_list.end(), lhs.name) - sorted_technique_list.begin()) < (std::find(sorted_technique_list.begin(), sorted_technique_list.end(), rhs.name) - sorted_technique_list.begin());
        });
    ...
    for (technique& technique : _techniques) {
        ...
        if (technique.annotation_as_int("enabled") || std::find(technique_list.begin(), technique_list.end(), technique.name) != technique_list.end())
            enable_technique(technique);
        else
            disable_technique(technique);
...
bool reshade::runtime::switch_to_next_preset(std::filesystem::path filter_path, bool reversed)
{
    ...
    for (std::filesystem::path preset_path : std::filesystem::directory_iterator(filter_path, std::filesystem::directory_options::skip_permission_denied, ec)) {
        ...
        if (filter_text.empty() || std::search(preset_name.begin(), preset_name.end(), filter_text.native().begin(), filter_text.native().end(), [](wchar_t c1, wchar_t c2) { return towlower(c1) == towlower(c2); }) != preset_name.end())
            preset_paths.push_back(std::move(preset_path));
    }
...
void reshade::runtime::save_screenshot(const std::wstring& postfix, const bool should_save_preset)
{
    ...
    if (!_screenshot_save_success) {
        ...
        std::error_code ec;
        std::filesystem::copy_file(_current_preset_path, screenshot_path.replace_extension(L".ini"), std::filesystem::copy_options::overwrite_existing, ec);
    }
}
